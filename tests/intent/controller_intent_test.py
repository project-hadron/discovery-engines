...
class ControllerIntentTest(unittest.TestCase):
    ...
    def test_register_task(self):
        ...
        dc = self.instance
        synth_df = dc.run_synthetic(task_name='hk_income', size=1000, uri_pm_repo=repo_uri, run_task=True)
        result = dc.run_transition( canonical=synth_df, task_name='hk_income', uri_pm_repo=repo_uri, run_task=True, intent_order=2)
        ...
        self.assertEqual(['run_synthetic'], list(intent.get('primary_swarm').get('0').keys()))
        self.assertEqual(['run_transition'], list(intent.get('primary_swarm').get('2').keys()))
        # run the pipeline
